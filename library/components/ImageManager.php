...
class ImageManager extends BaseFileManager
{
    ...
    public function __construct($config = array())
    {
        ...
        $this->uploadPath = UsniAdaptor::app()->getAssetManager()->imageUploadPath;
        $this->thumbnailUploadUrl = UsniAdaptor::app()->getAssetManager()->getThumbnailUploadUrl();
        $this->uploadUrl = UsniAdaptor::app()->getAssetManager()->getImageUploadUrl();
        $this->thumbnailUploadPath = UsniAdaptor::app()->getAssetManager()->thumbUploadPath;
    ...
    public function getThumbnailImage()
    {
        ...
        return UiHtml::img($url, ['width' => $this->thumbWidth, 'height' => $this->thumbHeight, 'class' => $this->cssClass]);
    }
    ...
    {
        $defaultWidth       = $this->defaultThumbWidth;
        $defaultHeight      = $this->defaultThumbHeight;
        $width              = $this->thumbWidth != null ? $this->thumbWidth : $defaultWidth;
        $height             = $this->thumbHeight != null ? $this->thumbHeight : $defaultHeight;
        return [$width, $height];
    }
     ...
     * @return boolean
     */
    public function checkIfImageExists($isThumbnail = true)
    {
        if($isThumbnail === true)
        {
            $path   = $this->thumbnailUploadPath;
            $prefix = $this->getFilePrefix();
        }
        else
        {
        ...
        return FileUploadUtil::checkIfFileExists($path, $this->model->{$this->attribute}, $prefix);
    }
    ...
    public function getNoAvailableImage($htmlOptions = [])
    {
        ...
        return UiHtml::img($url, ['width' => $width, 'height' => $height]); 
    }
}