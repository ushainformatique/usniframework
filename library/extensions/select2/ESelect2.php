...
class ESelect2 extends InputWidget
{
    ...
    /**
     * @var array UiHtml::dropDownList $data param
     */
    public $data = array();
    /**
    ...
    protected function renderDropdownList()
    {
        if ($this->hasModel())
        {
            ...
            echo UiHtml::activeDropDownList($this->model, $this->attribute, $this->data, $this->options);
        }
        else
        {
            $this->options['id'] = $this->id;
            echo UiHtml::dropDownList($this->name, $this->value, $this->data, $this->options);
        }
    }
    ...
    protected function renderToggleAll()
    {
        ...
        $prefix = 's2-togall-';
        UiHtml::addCssClass($options, "{$prefix}select");
        UiHtml::addCssClass($sOptions, "s2-select-label");
        UiHtml::addCssClass($uOptions, "s2-unselect-label");
        $options['id'] = $prefix . $this->options['id'];
        $labels = UiHtml::tag('span', $settings['selectLabel'], $sOptions) .
            UiHtml::tag('span', $settings['unselectLabel'], $uOptions);
        $out = UiHtml::tag('span', $labels, $options);
        echo UiHtml::tag('span', $out, ['id' => 'parent-' . $options['id'], 'style' => 'display:none']);
    }
}
